...
(function (global) {
    var req, s, head, baseElement, dataMain, src, interactiveScript, currentlyAddingScript, mainScript, subPath,
        version = "2.1.0", commentRegExp = /(\/\*([\s\S]*?)\*\/|([^:]|^)\/\/(.*)$)/mg,
    ...
    function each(ary, func) {
        if (ary) for (var i = 0; i < ary.length; i += 1) if (ary[i] && func(ary[i], i, ary)) break
    }

    function eachReverse(ary, func) {
        if (ary) for (var i = ary.length - 1; i > -1; i -= 1) if (ary[i] && func(ary[i], i, ary)) break
    }
    ...
    function eachProp(obj, func) {
        var prop;
        for (prop in obj) if (obj.hasOwnProperty(prop)) if (func(obj[prop], prop)) break
    }
    ...
    function newContext(contextName) {
        var inCheckLoaded, Module, context, handlers, checkLoadedTimeoutId,
            config = {waitSeconds: 7, baseUrl: "./", paths: {}, pkgs: {}, shim: {}}, registry = {}, undefEvents = {},
            defQueue = [], defined = {}, urlFetched = {}, requireCounter = 1, unnormalizedCounter = 1;

        function trimDots(ary) {
            for (var part, i = 0; ary[i]; i += 1) {
                part = ary[i];
                if (part === ".") {
                    ary.splice(i, 1);
                    i -= 1
        ...
        function normalize(name, baseName, applyMap) {
            var pkgName, pkgConfig, mapValue, nameParts, i, j, nameSegment, foundMap, foundI, foundStarMap, starI,
                baseParts = baseName && baseName.split("/"), normalizedBaseParts = baseParts, map = config.map,
                starMap = map && map["*"];
            if (name && name.charAt(0) === ".") if (baseName) {
                if (config.pkgs[baseName]) normalizedBaseParts = baseParts = [baseName]; else normalizedBaseParts = baseParts.slice(0, baseParts.length - 1);
        ...
        function on(depMap, name, fn) {
            var id = depMap.id, mod = registry[id];
            if (hasProp(defined, id) && (!mod || mod.defineEmitComplete)) name === "defined" && fn(defined[id]); else getModule(depMap).on(name, fn)
        }
        ...
        handlers = {
            "require": function (mod) {
                ...
                if (mod.map.isDefine) return mod.exports ? mod.exports : (mod.exports = defined[mod.map.id] = {})
            }, module: function (mod) {
        ...
        function breakCycle(mod, traced, processed) {
            ...
            if (mod.error) mod.emit("error", mod.error); else {
                ...
                each(mod.depMaps, function (depMap, i) {
                    var depId = depMap.id, dep = registry[depId];
                    if (dep && !mod.depMatched[i] && !processed[depId]) if (traced[depId]) {
        ...
        Module.prototype = {
            init: function (depMaps, factory, errback, options) {
                ...
                var map = this.map;
                if (this.shim) context.makeRequire(this.map, {enableBuildCallback: true})(this.shim.deps || [], bind(this, function () {
                ...
                on(pluginMap, "defined", bind(this, function (plugin) {
                    var load, normalizedMap, normalizedMod, name = this.map.name,
                        ...
                        localRequire = context.makeRequire(map.parentMap, {enableBuildCallback: true, skipMap: true});
                    ...
                    load.error = bind(this, function (err) {
                        ...
                        eachProp(registry, function (mod) {
                            mod.map.id.indexOf(id + "_unnormalized") === 0 && cleanRegistry(mod.map.id)
                        });
                        onError(err)
                    });
        ...
        function removeListener(node, func, name, ieName) {
            if (node.detachEvent && !isOpera) ieName && node.detachEvent(ieName, func); else node.removeEventListener(name, func, false)
        }
        ...
        context = {
            ...
            configure: function (cfg) {
                ...
                if (cfg.map) config.map = mixin(map || {}, cfg.map, true, true);
                if (cfg.shim) {
                    eachProp(cfg.shim, function (value, id) {
                        if (isArray(value)) value = {deps: value};
                        if (value.exports && !value.exportsFn) value.exportsFn = context.makeShimExports(value);
                        shim[id] = value
                    });
                    config.shim = shim
                }
            ...
            makeRequire: function (relMap, options) {
                ...
                function require(deps, callback, errback) {
                    ...
                    if (typeof deps === "string") {
                        ...
                        if (relMap && handlers[deps]) return handlers[deps](registry[relMap.id]);
                        if (req.get) return req.get(context, deps, relMap);
                        map = makeModuleMap(deps, relMap, false, true);
                        id = map.id;
                        return !hasProp(defined, id) ? onError(makeError("notloaded", 'Module name "' + id + '" has not been loaded yet for context: ' + contextName + (relMap ? "" : ". Use require([])"))) : defined[id]
                    }
                    takeGlobalQueue();
                ...
                if (!relMap) require.undef = function (id) {
                    takeGlobalQueue();
                    var map = makeModuleMap(id, relMap, true), mod = registry[id];
                    delete defined[id];
            ...
            completeLoad: function (moduleName) {
                var found, args, mod, shim = config.shim[moduleName] || {}, shExports = shim.exports;
                takeGlobalQueue();
                while (defQueue.length) {
                    args = defQueue.shift();
                    if (args[0] === null) {
                ...
                if (!found && !defined[moduleName] && mod && !mod.inited) if (config.enforceDefine && (!shExports || !getGlobal(shExports))) return hasPathFallback(moduleName) ? void 0 : onError(makeError("nodefine", "No define call for " + moduleName, null, [moduleName])); else callGetModule([moduleName, shim.deps || [], shim.exportsFn]);
                checkLoaded()
            },
    ...
    define = function (name, deps, callback) {
        ...
        if (!deps.length && isFunction(callback)) if (callback.length) {
