...
$('.btn-submit').click(function () {
  ...
  if (!oldpassword) {
    _progress.shared.showMessage('请填写原密码');
    return;
  }

  if (!newpassword) {
    _progress.shared.showMessage('请填写新密码');
    return;
  }

  if (newpassword != repeat) {
    _progress.shared.showMessage('两次输入不一致');
    return;
  }
  ...
  (0, _fetch2['default'])({
    ...
    success: function success() {
      _progress.shared.showMessage('修改成功');
      setTimeout(function () {
        location.reload();
      }, 400);
    },
...
/******/ });
//# sourceMappingURL=//dn-piaoniu-static.qbox.me/pc/user/password.js.34176b9e.map